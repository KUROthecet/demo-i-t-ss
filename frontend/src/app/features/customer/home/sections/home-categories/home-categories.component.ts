...
class CardParallaxHandler {
  ...
  onMouseMove(e: MouseEvent): void {
    ...
    const fg = this.card.querySelector<HTMLElement>('.home-categories__card-fg-img');
    const bg = this.card.querySelector<HTMLElement>('.home-categories__card-glow');
...
export class HomeCategoriesComponent implements AfterViewInit, OnDestroy {
  ...
  private scrollInnerEl: HTMLElement | null = null;

  readonly titleLine1Chars: string[] = 'Amazing'.split('');
  readonly titleLine2Chars: string[] = 'Worth Keeping'.split('');
  ...
  private registerGsapAnimations(): void {
    document.fonts.ready.then(this.initTitleAnimations.bind(this));
  ...
  private initTitleAnimations(): void {
    ...
    gsap.timeline({
      scrollTrigger: {
        trigger: sectionEl,
        start: 'top top',
        end:   this.computeTitleScrollEnd.bind(this, innerEl),
        scrub: true,
  ...
  private computeTitleScrollEnd(innerEl: HTMLElement): string {
    return `+=${(innerEl?.scrollWidth ?? 0) - window.innerWidth}`;
  }

  private setupHorizontalScrollNow(sectionEl: HTMLElement, innerEl: HTMLElement): void {
    requestAnimationFrame(this.buildHorizontalScroll.bind(this, sectionEl, innerEl));
  }

  private buildHorizontalScroll(sectionEl: HTMLElement, innerEl: HTMLElement): void {
    const scrollAmount = innerEl.scrollWidth - window.innerWidth;
    if (scrollAmount <= 0) return;

    this.scrollInnerEl = innerEl;

    gsap.to(innerEl, {
      x:    this.computeScrollX.bind(this),
      ease: 'none',
      scrollTrigger: {
        trigger: sectionEl,
        start: 'top top',
        end:   this.computeScrollEnd.bind(this),
        scrub: 1.5,
        pin: true,
        anticipatePin: 1,
        invalidateOnRefresh: true
  ...
  private computeScrollX(): number {
    if (!this.scrollInnerEl) return 0;
    return -(this.scrollInnerEl.scrollWidth - window.innerWidth);
  }

  private computeScrollEnd(): string {
    if (!this.scrollInnerEl) return '+=0';
    return `+=${(this.scrollInnerEl.scrollWidth - window.innerWidth) * 1.5}`;
  }
  ...
  private initHorizontalScroll(): void {
    ...
    setTimeout(this.setupHorizontalScrollNow.bind(this, sectionEl, innerEl), 100);
  }

  private registerCardParallax(card: HTMLElement): void {
    new CardParallaxHandler(card).register();
  }

  private initCardParallax(): void {
    const cards = document.querySelectorAll<HTMLElement>('.home-categories__card');
    for (const card of Array.from(cards)) {
      this.registerCardParallax(card);
    }
  }
