...
export class HomeBenefitsComponent implements AfterViewInit, OnDestroy {
  ...
  readonly paraWords: string[] =
    'No hidden fees. Authentic physical copies only. Straight from our shelves to your front door.'.split(' ');
  ...
  private initAnimations(): void {
    ...
    for (const benefit of this.benefits) {
      this.animateBenefitCard(tl, benefit.cssClass);
    }
  }

  private animateBenefitCard(tl: gsap.core.Timeline, cssClass: string): void {
    tl.to(`.${cssClass}`, {
      duration: 1,
      opacity: 1,
